...
- `runtime_backend=firecracker` executes through the in-process [`microvm-runtime`](https://github.com/tangle-network/microvm-runtime) driver consumed directly by `sandbox-runtime`. The operator is the Firecracker host; there is no separate host-agent service. The L1 job/operator API surface is unchanged. The create path returns a real host-reachable sidecar endpoint, installs per-VM iptables DNAT rules for `metadata_json.ports`, sizes the per-VM ext4 rootfs via `RootfsRegistry::clone_for_vm_with_size` when `disk_gb > 0`, and injects the full per-VM env table plus a host-minted sidecar auth token into the guest over vsock via the `GuestMetadataClient`. No `SandboxError::Unsupported` paths remain in the firecracker backend.
