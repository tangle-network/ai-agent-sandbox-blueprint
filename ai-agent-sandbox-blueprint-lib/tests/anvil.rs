...
// NOTE: the previous `runs_firecracker_jobs_end_to_end` test exercised the
// host-agent HTTP boundary that no longer exists. Firecracker provisioning
// now goes through the in-process `microvm-runtime` driver, and the driver's
// alpha release does not yet expose a host-reachable sidecar endpoint — so a
// real end-to-end job-path test against a microVM cannot pass yet. It will
// return once `microvm-runtime` ships network setup; see
// `sandbox-runtime/tests/firecracker_in_process.rs` for the current
// invariants that pin the wrapper's `Unsupported` contract.
