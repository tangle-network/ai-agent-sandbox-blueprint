...
async fn sidecar_image_drift_handler(SessionAuth(address): SessionAuth) -> impl IntoResponse {
...
async fn upgrade_sandbox_image_handler(
    SessionAuth(address): SessionAuth,
    Path(sandbox_id): Path<String>,
) -> impl IntoResponse {
    if let Err(e) = require_managing_operator(&address) {
...
async fn upgrade_stale_sidecar_images_handler(
    SessionAuth(address): SessionAuth,
) -> impl IntoResponse {
    if let Err(e) = require_managing_operator(&address) {
        return e.into_response();
